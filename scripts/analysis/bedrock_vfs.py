...
def filter_events_by_percent_range(
    events: List[Dict[str, any]], start_pct: float, end_pct: float
) -> List[Dict[str, any]]:
    ...
    if not (0.0 <= start_pct < end_pct <= 100.0):
        raise ValueError(
            "Range must satisfy 0 <= range-start-pct < range-end-pct <= 100"
        )
...
def bucketize(
    ...
    first_ts = (
        base_ts if base_ts is not None else min(event["timestamp"] for event in events)
    )
...
def main():
    ...
    if not events:
        raise SystemExit(
            "No parseable VFS read/write events were found in the trace logs."
        )
