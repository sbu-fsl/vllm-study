...
METRIC_LINE_RE = re.compile(
    r"^vllm:(?P<name>[A-Za-z0-9_]+)\s*=\s*(?P<value>[-+]?\d*\.?\d+)$"
)
SUMMARY_START_RE = re.compile(
    r"^---\s+.*:\s+\d+\s+requests,\s+\d+\s+ok,\s+\d+\s+failed\s+---$"
)
SUMMARY_END_RE = re.compile(r"^---\s+end time:")
SUMMARY_NUMBER_RE = re.compile(
    r"^(?P<name>[A-Za-z0-9 %_.-]+):\s*(?P<value>[-+]?\d*\.?\d+)"
)

# kv stored metric is not directly logged by vLLM, but we can compute it as the difference between
# prompt tokens and prefill computed tokens
...
def parse_log_metrics(log_file_path):
    ...
    with log_file_path.open("r", encoding="utf-8", errors="replace") as handle:
        for raw_line in handle:
            ...
            if in_summary:
                if metric_match:
                    summary_metrics[metric_match.group("name")] = float(
    ...
    if (
        PROMPT_TOKENS_METRIC in summary_metrics
        and PREFILL_COMPUTED_METRIC in summary_metrics
    ):
...
def plot_summary_metric_for_log(
    ...
    file_name = f"{sanitize_filename(Path(log_name).stem)}__summary__{sanitize_filename(metric_name)}.png"
    output_path = output_dir / file_name
    plt.savefig(output_path, dpi=160)
...
def plot_summary_star_plot(log_name, model_to_summary_metrics, output_dir, show_plots):
    ...
    for summary_metrics in model_to_summary_metrics.values():
        all_metrics.update(summary_metrics.keys())

    all_metrics = sorted(all_metrics)
    model_names = sorted(model_to_summary_metrics.keys())
...
def build_plots(models_root, model_glob, output_dir, show_plots):
    ...
    for log_name, model_to_file in sorted(grouped_logs.items()):
        ...
        for metric_name in sorted(all_metric_names):
            ...
            plot_metric_for_log(
                log_name, metric_name, model_series, output_dir, show_plots
            )

        if all_summary_names:
            print(
                f"Plotting {len(all_summary_names)} summary metric(s) for {log_name}."
            )
            plot_summary_star_plot(
                log_name, model_to_summary_metrics, output_dir, show_plots
            )
...
def main():
    ...
    if not models_root.exists() or not models_root.is_dir():
        raise SystemExit(
            f"models_dir does not exist or is not a directory: {models_root}"
        )
